...
//
// AARK PROTOCOL COPYRIGHT (C) 2022
...
contract Aark is ERC20Detailed, Ownable {
    ...
    string public _name = "AARK";
    string public _symbol = "$Arkp";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 55 * 10**5 * 10**DECIMALS;   // 5,500,000 tokens

    uint256 public liquidityFee = 40;       // 4%
    uint256 public treasuryFee = 35;        // 3.5%
    uint256 public InsuranceFundFee = 50;   // 5%
    uint256 public sellFee = 0;   	//0%
    uint256 public firePitFee = 25;         //2,5%
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(InsuranceFundFee).add(firePitFee);   // 15%
    ...
    uint256 private constant MAX_SUPPLY = 325 * 10**7 * 10**DECIMALS;   // 3,250,000,000 tokens
    ...
    constructor() ERC20Detailed("AARK", "$Arkp", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0xC2A9E7bE64bd00f1C60a2156Ab4CEA522075f44a;  //TODO: need to confirm again 
        treasuryReceiver =      0x7Dec37CC8681E4AF23fb9Fa8e7A69589e782f939;
        InsuranceFundReceiver = 0x344251C125E79FE93cA2141708A9F1bBCB531B74;
    ...
    function rebase() internal {
        ...
        if ( inSwap ) return;
        uint256 rebaseRate = 4476897;       //APY = 380,025% 
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no AARK token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
