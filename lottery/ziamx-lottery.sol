...
contract ZiMaxLottery is ReentrancyGuard, IZMXloniaLottery, Ownable {
    ...
    uint256 private maxPriceTicketInZMX =1000 ether;
    uint256 private minPriceTicketInZMX = 0.005 ether;

    uint256 public pendingInjectionNextLottery;

    uint256 private constant MIN_DISCOUNT_DIVISOR = 300;
    uint256 public constant MIN_LENGTH_LOTTERY = 4 hours - 5 minutes; // 4 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 4 days; //15 minutes;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInZMX;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] ZMXPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInZMX;
        uint32 finalNumber;
    }
    ...
    constructor(address _ZMXTokenAddress, address _randomGeneratorAddress) {
        ZMXToken = IERC20(_ZMXTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ZMX to this contract
        uint256 amountZMXToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInZMX,
            _ticketNumbers.length
        );

        // Transfer ZMX tokens to this contract
        ZMXToken.safeTransferFrom(address(msg.sender), address(this), amountZMXToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInZMX += amountZMXToTransfer;
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInZMXToTransfer += rewardForTicketId;
        }

        ZMXToken.safeTransfer(msg.sender, rewardInZMXToTransfer);
        emit TicketsClaim(msg.sender, rewardInZMXToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInZMX) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].ZMXPerBracket[j] =
                ...
                // A. No ZMX to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].ZMXPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInZMX - amountToShareToWinners);

        ZMXToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        ZMXToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInZMX += _amount;

        emit LotteryInjection(_lotteryId, _amount);
    }
    // ["1452895","1457577","1237495","1458605","1658915"]
    //  ["250","375","625","1250","2500","5000"] => [2.50%, 3.75%, 6.25%, 12.50%, 25%, 50%]
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInZMX >= minPriceTicketInZMX) && (_priceTicketInZMX <= maxPriceTicketInZMX),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInZMX: _priceTicketInZMX,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            ZMXPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInZMX: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(ZMXToken), "Cannot be ZMX token");

        IERC20(_tokenAddress).safeTransfer(address(msg.sender), _tokenAmount);

        emit AdminTokenRecovery(_tokenAddress, _tokenAmount);
    }
    function setMinAndMaxTicketPriceInZMX(uint256 _minPriceTicketInZMX, uint256 _maxPriceTicketInZMX)
        external
        onlyOwner
    {
        require(_minPriceTicketInZMX <= _maxPriceTicketInZMX, "minPrice must be < maxPrice");

        minPriceTicketInZMX = _minPriceTicketInZMX;
        maxPriceTicketInZMX = _maxPriceTicketInZMX;
    }
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].ZMXPerBracket[_bracket];
        } else {
            return 0;
        }
    }
    ...
    function getTime(uint256 lotteryId_) public view returns(uint256 _time){
        if(block.timestamp <= _lotteries[lotteryId_].endTime){
            _time =  (((_lotteries[lotteryId_].endTime) -(block.timestamp))/(60 seconds));
        }
        else{
            _time = 0;
        }
